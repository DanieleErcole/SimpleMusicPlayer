...
import it.danieleercole.soundbox.ui.components.Divider
import it.danieleercole.soundbox.ui.state.DialogsVM
import it.danieleercole.soundbox.utils.formatInstantToHuman
import it.danieleercole.soundbox.utils.formatTimestamp
                ...
                Surface {
                    Column(
                        ...
                        Column(
                            ...
                            Divider(modifier.padding(vertical = 8.dp))
                            InfoField(title = stringResource(R.string.duration), text = formatTimestamp(
                                track.internal.durationMs
                            )
                            )
                            Divider(modifier.padding(vertical = 8.dp))
                            InfoField(title = stringResource(R.string.added_date), text = formatInstantToHuman(
                                track.internal.addedToLibrary
                            )
                            )
                            InfoField(title = stringResource(R.string.last_played_date), text = track.internal.lastPlayed?.let {
                                formatInstantToHuman(
                                    it
                                )
                            }
